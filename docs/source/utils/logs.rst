...
.. code-block:: bash
   ...
   lava utils logs job.yaml

   # Only show device console output
   lava utils logs job.yaml --filters target

Output
------

The formatted log lines are printed to standard output. Both the native LAVA
YAML array format and the JSON envelope produced by ``lava jobs logs`` are
